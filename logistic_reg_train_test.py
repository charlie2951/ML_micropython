...
#that 1st row which contain string is removed (string not supported)
...
def csvread(file_name):  # function for reading csv file
    ...
    return file_data
####### Test function ###########################
raw_data = csvread('diabetes_pima.csv')

# Normalize data using mean and stdev()
scaled_data = [lm.normalize(raw_data[i]) for i in range(len(raw_data[:8]))]
xtrain, xtest, ytrain, ytest = lm.train_test_split(scaled_data,raw_data[8], 0.7)#70% data used for training
# Xtrain data
#Train and Build model using Logistic Regression
w, b, loss_train, ypred_train = lm.logistic_regressor(xtrain, ytrain, 0.1, 1000)
